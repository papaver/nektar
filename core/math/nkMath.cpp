...
NK_MATH_INLINE bool IsPowerOfTwo(u32 v)
{
  bool is_power = v && !(v & (v - 1));
  return is_power;
}
...
NK_MATH_INLINE u32 GetHighestPowerOfTwo(u32 v)
{
  --v;
  v |= v >> 1;
  v |= v >> 2;
...
//------------------------------------------------------------------------------
// can be used to calculate next boundry when working with bits
//  add p-1 to cause overflow unless on boundry already then mask out p-1 bits
//------------------------------------------------------------------------------

NK_MATH_INLINE u32 GetHighestMultipleOfPowerOfTwo(u32 v, u32 p)
{
  NK_ASSERT(IsPowerOfTwo(p));

  u32 multiple = (v + (p - 1)) & ~(p - 1);
  return multiple;
}
